  ...
  return (
    <div className="px-12 bg-gray-100 pt-8 mx-auto dark:bg-primary">
      ...
      <div className="bg-white h-96 shadow-sm w-full">
        <Image className="h-full w-full" src={event.href} height={100} width={100} alt='banner'/>
      </div>:
      <div className="bg-white h-96 shadow-sm w-full">
        <Image className="h-full w-full" src="https://eventscal.lau.edu.lb/students/images/codebanner.jpg" height={100} width={100} alt='banner'/>
      </div>}
        ...
        <div className="flex flex-col gap-2 w-2/3 h-screen overflow-y-auto">
          <div className="p-5 bg-white shadow-sm">
            ...
            <div className="text-sm font-medium flex ml-2 gap-2 mt-4 items-center">
              ...
              <p><b>Updated On :</b> {event.updatedAt.toUTCString().substring(0,16)}</p>
            </div>
          </div>
          <div className="sticky top-0 flex items-center pl-10 p-5 shadow-sm gap-6 text-zinc-600 h-20 bg-white z-10">
            <h1 className={`text-sm font-medium cursor-pointer p-2 px-3 rounded ${activeComponent==='Component1'?"bg-primary-600 text-white":"hover:bg-slate-200"}`} onClick={() => setActiveComponent('Component1')}>Details</h1>
            <h1 className={`text-sm font-medium cursor-pointer p-2 px-3 rounded ${activeComponent==='Component2'?"bg-primary-600 text-white":"hover:bg-slate-200"}`} onClick={() => setActiveComponent('Component2')}>Timeline</h1>
            <h1 className={`text-sm font-medium cursor-pointer p-2 px-3 rounded ${activeComponent==='Component3'?"bg-primary-600 text-white":"hover:bg-slate-200"}`} onClick={() => setActiveComponent('Component3')}>Prizes</h1>
          </div>
          ...
          <div className="bg-white p-4 pt-4 rounded shadow-sm">
            ...
            <div className="flex items-center gap-4 mt-8">
              ...
              <div className='flex flex-col gap-1'>
                ...
                <p className="text-sm font-medium">{event.minTeamSize===event.maxTeamSize?event.minTeamSize:event.minTeamSize+' - '+event.maxTeamSize}</p>
              </div>
            </div>
